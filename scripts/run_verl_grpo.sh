#!/bin/bash
# Run GRPO training with verl (4 GPU, vLLM rollout, FSDP training)
set -e
...
train_df = pd.DataFrame(verl_data[:len(verl_data)-100])
val_df = pd.DataFrame(verl_data[len(verl_data)-100:])

train_df.to_parquet("verl_train.parquet", index=False)
val_df.to_parquet("verl_val.parquet", index=False)
...
python -m agentlightning.verl \
    ...
    trainer.experiment_name=metacot-verl-grpo-4gpu \
    trainer.nnodes=1 \
