...
export WANDB_NAME="${WANDB_NAME:-verl_e21r_recovery_0410}"
...
pkill -f 'verl.trainer.main_ppo.*verl_e21r_recovery_0410' 2>/dev/null || true
...
nohup python -m verl.trainer.main_ppo \
  ...
  data.train_files=data/verl_train_redirect.parquet \
  data.val_files=data/verl_val_redirect.parquet \
  data.max_prompt_length=512 \
  data.max_response_length=1536 \
  data.train_batch_size=32 \
  algorithm.adv_estimator=gdpo \
  ++algorithm.gdpo_reward_keys='[correctness,confidence_revision,redirect_execution,verify_execution,meta_floor,meta_count_bonus]' \
  algorithm.use_kl_in_reward=False \
  ++reward.custom_reward_function.path=src/training/verl_reward.py \
  ++reward.custom_reward_function.name=compute_score_confidence_centered \
  ...
  trainer.experiment_name=verl_e21r_recovery_0410 \
  trainer.logger='[console,wandb]' \
  trainer.nnodes=1 \
  trainer.n_gpus_per_node=4 \
  trainer.save_freq=100 \
  trainer.test_freq=50 \
  > /scratch/verl_e21r_recovery_0410.log 2>&1 < /dev/null &

echo $! > /scratch/verl_e21r_recovery_0410.pid
echo STARTED_E21R $(cat /scratch/verl_e21r_recovery_0410.pid)
