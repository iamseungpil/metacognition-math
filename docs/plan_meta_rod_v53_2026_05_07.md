...
- **2026-05-07 (v5.5 update)** — Second codex review found that v5.4's `_content_kl_at_injection_points` did NOT actually inject META_START — both teacher forwards used identical inputs (semantically collapsed despite two physical calls). Plan §4.2's "two queries" intent therefore not implemented. **v2 fix (R10)**: implement TRUE META injection. For each rollout pick one random non-meta body position p, build `T_content_input = prompt + gold + completion[:p] + META_START + completion[p:]`, run a separate teacher forward, compute KL at the position right after the injected META. Cost: 1 extra teacher forward per batch (vs 1 in v1). Plan §4.2 docstring clarified to make injection explicit. R9 (v2 collapsed) cancelled, R10 (v2 true-injection) submitted. Forecast unchanged: AIME 24–31.
