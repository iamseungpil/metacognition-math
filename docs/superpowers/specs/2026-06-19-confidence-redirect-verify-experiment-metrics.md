...
- **confidence thresholds — single source of truth:** `src/data/confidence_label.py` owns `CONF_LO=0.30` / `CONF_HI=0.70` / `CONFWRONG_THR=0.60`. The build driver and the eval BOTH reference these (the eval re-exports them as `SHARED_CONF_LO` / `SHARED_CONF_HI` in `src/eval/redirect_verify_metrics.py`), so a problem is never bucketed one way at build and judged by a different number at eval.

### Held-out eval bars are deliberately STRICTER than the shared build thresholds
The held-out action-appropriateness test (C6) does NOT reuse the build's loose 0.30 low bar. It uses `HELDOUT_LOW_CONF = 0.5` (> `SHARED_CONF_LO = 0.30`) and `HELDOUT_HIGH_CONF = 0.70` (== `SHARED_CONF_HI`), declared in `src/eval/redirect_verify_metrics.py` with an assertion that they are never looser than the shared bars. Rationale: at build a problem can be redirect-bucketed at the 0.30 self-consistency floor, but to PASS the held-out "decided-from-confidence" test the model's emitted confidence must clear the stricter 0.5 bar — a held-out emission that merely squeaks under 0.30 is not credited. This is a documented, asserted strictening, not a silent divergence.
...
| C6 | **Action-appropriateness** — of fired meta blocks, fraction whose action is appropriate, where appropriateness is conditioned on BOTH situation AND emitted confidence: **redirect** appropriate iff `recoverable_wrong` (gold) **AND** emitted `confidence < HELDOUT_LOW_CONF` (0.5) — the model redirected *because* it was unsure, symmetric to **verify** (appropriate iff `confidence ≥ HELDOUT_HIGH_CONF` = 0.70). A high-confidence redirect on a recoverable-wrong case is a **misfire**, NOT free credit (gold-alone crediting would invert the decide-FROM-confidence north-star). Reported alongside a separate **action↔confidence consistency rate** (fraction of fired actions whose emitted confidence sits on the side its action implies, measured independently of gold). | **≥ 0.80** | < 0.5 → STOP (action decoupled from confidence) | `action_appropriateness(records, high_conf=HELDOUT_HIGH_CONF, low_conf=HELDOUT_LOW_CONF)` from `src/eval/redirect_verify_metrics.py`; join emitted action+conf vs measured band on held-out |
