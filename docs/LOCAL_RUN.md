...
전부 있다. 다만 아래 **갭 4개는 현재 턴키가 아니다**: ① 전 config/스크립트가
`/scratch/...` 경로 하드코딩, ② init 모델·RL parquet이 소유자 private HF
repo(`iamseungpil/metacot`, `iamseungpil/metacot-sdc-data`)에 있어 read 토큰
필요, ③ 로컬 런처 스크립트가 없음(실행 시퀀스가 amlt yaml `command:` 블록
안에만 존재), ④ conda-pack tarball(`env_snapshots/simplerl_v4.tar.gz`)의
빌드 레시피가 repo에 미문서. 갭은 숨기지 않고 각 절에 "현재 불가/수동필요"로
표기한다.
...
- **토큰**: `HF_TOKEN`(소유자에게 `iamseungpil/metacot` +
  `iamseungpil/metacot-sdc-data` read 권한 요청 — **수동필요**, .env.example
  없음), `WANDB_API_KEY`(선택 — 없으면 아래처럼 `trainer.logger=['console']`
  오버라이드). 토큰 값은 `.env`에만 두고 문서·코드에 절대 기록 금지.
...
**parquet**: `scripts/pull_parquets.py`는 인자 없이 하드코딩된
`/scratch/metacognition/data`로 받는다 — **갭**. `/scratch` 쓰기가 안 되면
수동으로: private HF dataset `iamseungpil/metacot-sdc-data`에서
`verl_train_meta_mix.parquet`·`verl_val_meta_mix.parquet` 2개를
`hf_hub_download`로 `$WORK/data`에 받는다 (rq3 arm은 meta_mix 2개만 씀).
...
| # | 갭 | 심각도 | 우회로 |
|---|---|---|---|
| 1 | private HF 토큰(모델·parquet·conda-pack) | BLOCKER | 소유자에게 read 권한 요청 — 현재 수동필요 |
