...
def _mock_rollout(question: str, gold: str, n: int):
    """Return n student rollouts (text, is_correct). Deterministic per problem."""
    if question.startswith("P1"):  # low conf: 1/4 correct -> redirect
        return [
            ("<think>wrong P1 attempt one</think> The answer is $3$.", False),
            ("<think>wrong P1 attempt two</think> The answer is $4$.", False),
            ("<think>wrong P1 attempt three</think> The answer is $2$.", False),
            ("<think>lucky P1</think> The answer is $7$.", True),
        ][:n]
    if question.startswith("P2"):  # high-conf-but-CHECKABLE: 3/4 correct -> verify
        ...
        return [
            ("<think>P2 solve</think> The answer is $42$.", True),
            ("<think>P2 solve</think> The answer is $42$.", True),
            ("<think>P2 solve</think> The answer is $42$.", True),
            ("<think>P2 slip</think> The answer is $41$.", False),
        ][:n]
    if question.startswith("P3"):  # hard, wrong -> would be redirect but dropped on difficulty
        return [("<think>P3 wrong</think> The answer is $1$.", False) for _ in range(n)]
    if question.startswith("P4"):  # low conf redirect, but teacher will fail
        return [
            ("<think>P4 wrong a</think> The answer is $1$.", False),
            ("<think>P4 wrong b</think> The answer is $2$.", False),
            ("<think>P4 wrong c</think> The answer is $3$.", False),
            ("<think>P4 wrong d</think> The answer is $4$.", False),
        ][:n]
    return [("<think>x</think> The answer is $0$.", False) for _ in range(n)]
...
def test_smoke_end_to_end(tmp_path: Path):
    ...
    # redirect row: confidence label ~ student pass rate (0.25) and loss-mask masks
    # the wrong prefix (some positions ignore_index, some trained).
    redirect_row = df[df["scenario"] == "redirect"].iloc[0]
    assert redirect_row["confidence_label"] == pytest.approx(0.25)
    mask = list(redirect_row["loss_mask"])
    assert any(v == -100 for v in mask), "wrong prefix must be masked"
    assert any(v == 1 for v in mask), "meta+recovery must be trained"
