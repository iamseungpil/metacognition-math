...
RQ3 mainline은 therefore RL-first가 아니라 self-distill-first로 재정렬해야 한다. 현재 claim-bearing path는 `strict SFT -> question_only_best_of_n -> correctness_only / correct_then_meta -> CE/SFT readout -> optional meta-only KL`이다. Retrieval과 `fixed_k_repair`는 side-evidence lane으로 남겨 두고, example bank가 없는 run은 retrieval evidence로 해석하지 않는다. 별도의 RL smoke lane은 historical E21R-v2를 수정하지 않고 `compute_score_e21r_v4_smoke` 기반 추가 실험으로 분리한다.
...
**Current status**: this is still open. The correct claim-bearing path is not “raw SFT data with meta preserved,” but `question_only_best_of_n` artifacts with explicit selector provenance. Retrieval is not active in the claim-bearing mainline by default, and `fixed_k_repair` remains side-evidence only unless an example bank is actually loaded and retrieval usage is non-zero.
...
1. `strict_base_sft -> question_only_best_of_n -> correctness_only -> naive self-distill`
2. `strict_meta_sft -> question_only_best_of_n -> correctness_only / correct_then_meta -> claim-bearing epistemic self-distill`
3. optional extension: `teacher top-k -> strict meta-only KL`
4. side-evidence only: separate GDPO smoke lane `compute_score_e21r_v4_smoke`
...
1. Build base `question_only_best_of_n` artifacts and train the naive lane.
2. Build meta `question_only_best_of_n` artifacts and train the claim-bearing epistemic lane.
3. Evaluate both against strict SFT baselines on controller retention, collapse metrics, and OOD accuracy.
4. Only if the meta lane shows preserved control and competitive OOD behavior, extend to teacher top-k + strict `meta_only` KL.
5. Run the new GDPO-v4 smoke lane separately from the self-distill table for later comparison.
